    ...
    <div class='span4 search-container'>
      <div class='search-form-container'>     
        <h4>I'm looking for:</h4>
				{{ Form::open("/search", "POST", array('id' => 'frm-search')); }}  
          <label>Property Type: </label>
           {{ Form::select('property_type', $property_type, '', array('id' => 'property-type')); }}
          <label>Minimum Price</label>
          <div class="input-prepend">
            <span class="add-on">Php</span>
            <input name='rent_min' class='input-large' type="text" placeholder="ex: 1000" id="rent-min">
          </div>
          <label>Maximum Price</label>
          <div class="input-prepend">
            <span class="add-on">Php</span>
            <input name='rent_max' class='input-large' type="text" placeholder="ex: 5000" id="rent-max">
          </div>
          <label>Search By</label>          
          <input name='address' type='text' class='input-xlarge' placeholder='Property Name, Address, Postal Code' id='address' />
          <br />
		  <input type="button" value="Search!"  class="btn btn-success"  style='margin-top -1%' id='btn-search'/>
			{{ Form::close(); }}
      </div> <!-- /#search -->   
    </div>
    <div class='span8'>
        ...
        <div class="carousel-inner">
          @if($carousel)
            ...
            <div class="item carousel-container">
              <img class='carousel-img'  src="/img/property/{{$img['property_id']}}/{{$img['name']}}" height="800" width="340">      
            ...
            <div class='span6'>
              <div class='row'>
                ...
                <div class='span3 property-details'>                  
                  <div class='property-name'>{{ HTML::link_to_action('crib@profile', $ad['property_name'], array('id' => $key, 'name' => urlencode($ad['property_name']))); }}</div>
...
$(function(){
  ...
  $('#btn-search').click(function(){
    ...
    if($('#rent-min').val() != '') {
      if(!num_regex.test($('#rent-min').val())) {     
        show_error('myalerts', 'Invalid! Minimum rent should be a number.');        
        isStepValid = false;                
      }
    }

    if($('#rent-max').val() != '') {
      if(!num_regex.test($('#rent-max').val())) {     
        show_error('myalerts', 'Invalid! Maximum Rent should be a number.');        
        isStepValid = false;                
      }
    }

    if($('#address').val() != '') {
      if(!words_regex.test($('#address').val())) {     
        show_error('myalerts', 'Invalid! Can only use alphanumeric, dot, space and comma.');        
        isStepValid = false;                
      }  
    }
    ...
    //  show_error('myalerts', html);
    }

  });
  function show_error(elem, message) {
    $("." + elem).addClass("error_message_bg");
    $("." + elem).append("<span class='validate error_message'>" + message + "</span>");
  }
});
</script>
@endsection