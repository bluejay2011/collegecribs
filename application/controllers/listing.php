...
class Listing_Controller extends Base_Controller {
	...
	public function action_index()
	{
		...
		if($_POST) {			
			...
			if($is_existing == 0) {
				...
				$details['map_long'] = $_POST['data']['long'];
				$details['map_lat'] = $_POST['data']['lat'];
				$details['rent_min'] = str_replace(",", "", $_POST['data']['rent_min']);
				$details['rent_max'] = str_replace(",", "", $_POST['data']['rent_max']);
				$details['zip_code'] = "0";       //TODO: Remove this $_POST['data']['zipcode'];
	...
	public function action_check_property_name()
	{
		if($_POST['name']) {			
			$is_existing = Properties::where('property_name', '=', $_POST['name'])->count();
			if($is_existing) {				
				return Response::json(array('result' => 'success', 'message' => "exist"));		
			}
			else  {
				return Response::json(array('result' => 'fail', "message" => 'exist'));		
			}
		}
		else {
			return Response::json(array('result' => 'fail', "message" => 'error'));		
		}
	}
