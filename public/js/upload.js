...
function displaySelectedFiles(){
	...
	if (!doc.getElementById("clear")) {
		var clear = doc.createElement("a");			
			clear.innerHTML = "Clear All Photos";
			clear.id = "clear";
			clear.className = "upload-buttons";

		//doc.body.appendChild(clear);
		$("#upload-buttons").append(clear);		

		clear.addEventListener("click", function(){
			//window.location.reload();
			$("#mypictures").children().remove();
			upload_data = [];
			//formdata = new FormData();
...
function createImage (source, fileobj, file) {
	...
	div2.className = "preview-thumbnail-remover";
	div2.innerHTML = "x";
	div.appendChild(div2);
	div.appendChild(element);

	// We store the file object as a property of the image (for use later)
	$('#mypictures').append(div);

	$(".preview-thumbnail-remover").click(function(){
		var name = $(this).next().attr("data-name");		
      	$(this).parent().remove();
      	delete upload_data[name];      	
      	formdata = new FormData();
    });
	...
	element.onload = function(){
		var x = modifyImageBeforeAppending(element, file);
		$(this).attr("data-name", x);
	}
}
...
function modifyImageBeforeAppending (img, file) {
	...
	*/
	...
	console.log("images_" + milliseconds);	
	...
	data["size"] = file.size;
	data["url"] = dataurl;
	upload_data["images_" + milliseconds] = data;	
	
	return "images_" + milliseconds;
}
...
function send()
{
	for (var key in upload_data) {
		//console.log(key);
		for (var prop in upload_data[key]) {
			//console.log(upload_data[key][prop]);
			formdata.append(key + "[]", upload_data[key][prop]);
		}
	}

	var xhr = new XMLHttpRequest();
	xhr.open("POST", "listing/test");
	xhr.send(formdata);
}